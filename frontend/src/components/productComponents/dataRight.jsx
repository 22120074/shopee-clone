...
function DataRight({
  ...
  const stock =
    product?.stockCounts.find((attr) => attr.attributeID === focusAttribute?.id)
      ?.quantity || 0;
  ...
  useEffect(() => {
    const found = product?.attributes.find(
      (attr) =>
        attr.nameEach === focusColor &&
        (attr.size === focusSize ||
          (focusSize === null && reducedSizes.length === 0)),
    );
  ...
  return (
    <div className="w-full min-w-0 flex flex-col flex-1">
      ...
      <div className="text-[19px] leading-[23px] line-clamp-2 w-full break-words">
        {product.favorite && (
          <span
            className="inline-block text-white bg-[#EE4D2D] 
      ...
      <div className="w-full md:w-auto flex flex-row items-center justify-center md:justify-start my-3">
        ...
        <div className="flex items-center gap-[2px] justify-start text-lg">
          ...
          <div className="">{formatSold(product.soldCount)}</div>
          <div
            className="w-[16px] h-[16px] text-[#767676] rounded-full border border-[#767676]
                    flex items-center justify-center leading-[16px] ml-2"
          >
            <i
              className="fa-solid fa-question text-[10px] text-[#767676]"
      ...
      <div className="w-full h-14 md:h-[64px] flex items-center justify-center md:justify-start bg-[#FAFAFA] gap-4 md:pl-6">
        {focusColor &&
        (focusSize || (focusSize === null && reducedSizes.length === 0)) &&
        focusAttribute ? (
          <div className="text-2xl md:text-xl lg:text-[30px] font-medium text-[#ee4d2d] flex items-center">
            ...
            {(
              (focusAttribute.price * (100 - product.discount)) /
              100
      ...
      {reducedAttributes.length > 0 && (
        <div className="w-full flex flex-row justify-start pl-6 mt-6">
          ...
          <div
            className="flex flex-row items-center justify-start flex-wrap gap-2 item-start w-[400px]
                        max-h-[130px] overflow-y-auto"
          >
            {reducedAttributes.map((attribute, index) => (
              <div
                key={index}
                className={`relative flex items-center h-[32px] md:h-9 lg:h-10 border p-2 gap-2
                                    ...
                                    ${
                                      !validAttribute.some(
                                        (item) =>
                                          item.nameEach === attribute.nameEach,
                                      ) && focusSize
              ...
              >
                ...
                <span className="text-sm lg:text-[15px] text-inherit">
                  {reducedAttributes[index]?.nameEach}
                </span>
                {focusColor === attribute.nameEach && (
                  <div
      ...
      {reducedSizes.length > 0 && (
        <div className="w-full flex flex-row justify-start pl-6 mt-6">
          ...
          <div
            className={`flex flex-row items-center justify-start flex-wrap gap-2 flex-1 max-w-[430px] 
                        max-h-[${40 * 3 + 8 * 2}px] overflow-y-auto`}
          >
            {reducedSizes.map((attribute, index) => (
              <div
                key={index}
                className={`relative flex items-center h-[32px] md:h-9 lg:h-10 border
      ...
      <div className="relative w-full flex flex-row justify-start pl-6 mt-6">
        ...
        <div className="flex items-center h-9 lg:h-10 border border-[#CCCCCC] text-sm md:text-[15px] text-inherit">
          <button
            className={`w-8 lg:w-10 h-full flex items-center justify-center
      ...
      <div className="w-full flex flex-row justify-evenly md:justify-start pl-0 md:pl-6 mt-8 md:mt-10 gap-4">
        ...
        <PrimaryButton
          ...
          width={isPhone ? "170px" : isIPad ? "180px" : "200px"}
          text="Mua Ngay"
        />
      </div>
    </div>
