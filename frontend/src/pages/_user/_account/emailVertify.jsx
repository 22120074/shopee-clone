...
} from "../../../features/api/authQuery";
import { useUpdateEmailMutation } from "../../../features/api/userQuery";
...
function EmailVertify() {
  ...
  const handleMouseEnter = () => {
    ...
    tl.to(iconRef.current, { x: 10, duration: 0.2, ease: "power1.out" }).to(
      iconRef.current,
      { x: 0, duration: 0.3, ease: "bounce.out" },
      ">",
    );
  ...
  const handleSubmit = async () => {
    ...
    try {
      focusInputOtp();
      await verifyOtpEmail({ email: user.email, otp }).unwrap();
      setCurrentStep(3);
    } catch (error) {
